...
import deliveryApproved from '../../delivery/clickEntregas/clickEntregasCreateReturnOrderService'
//import deliveryApproved from '../../delivery/clickEntregas/clickEntregasCreateReturnOrderServiceMock'
...
export default ({ config, db }) => {
    ...
    const requestStatusUpdateMapper = new RequestStatusUpdateMapper()
		await requestStatusUpdateMapper.update(request, RequestStatus.READY_TO_RETURN).catch( async (err) => {
      await deliveryCancelation(deliveryData.loggiOrderId, deliveryData.packageId, authData)
      console.log(err.message, err.data)
      res.status(STATUS_SERVER_ERROR).send(err.message)
      res.end()
    ...
    const requestLogMapper = new RequestLogMapper()
		await requestLogMapper.save(request, RequestStatus.READY_TO_RETURN).catch( async (err) => {
      await deliveryCancelation(deliveryData.loggiOrderId, deliveryData.packageId, authData)
      console.log(err.message, err.data)
      res.status(STATUS_SERVER_ERROR).send(err.message)
      res.end()
    ...
    const emailContent = emailHelper(
      "Delivery solicitado para deveolver serviço.", 
      request.clientName, 
      request.clientEmail,
      [
        "Informamos que o delivery foi acionado para devolver o seu pedido finalizado. Segue os dados dele:",
